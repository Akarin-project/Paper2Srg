...
public class CraftEntityEquipment implements EntityEquipment {
    ...
    private ItemStack getEquipment(EntityEquipmentSlot slot) {
        return CraftItemStack.asBukkitCopy(entity.getHandle().func_184582_a(slot));
    }

    private void setEquipment(EntityEquipmentSlot slot, ItemStack stack) {
        entity.getHandle().func_184201_a(slot, CraftItemStack.asNMSCopy(stack));
    }
    ...
    private void setDropChance(EntityEquipmentSlot slot, float chance) {
        if (slot == EntityEquipmentSlot.MAINHAND || slot == EntityEquipmentSlot.OFFHAND) {
            ((EntityLiving) entity.getHandle()).field_82174_bp[slot.func_188454_b()] = chance - 0.1F;
        } else {
            ((EntityLiving) entity.getHandle()).field_184655_bs[slot.func_188454_b()] = chance - 0.1F;
        }
    }

    private float getDropChance(EntityEquipmentSlot slot) {
        if (slot == EntityEquipmentSlot.MAINHAND || slot == EntityEquipmentSlot.OFFHAND) {
            return ((EntityLiving) entity.getHandle()).field_82174_bp[slot.func_188454_b()] + 0.1F;
        } else {
            return ((EntityLiving) entity.getHandle()).field_184655_bs[slot.func_188454_b()] + 0.1F;
        }
    }
}
