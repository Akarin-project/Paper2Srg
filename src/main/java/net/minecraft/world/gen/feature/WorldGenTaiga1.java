        ...
        if (blockposition.getY() >= 1 && blockposition.getY() + i + 1 <= 256) {
            ...
            for (int l1 = blockposition.getY(); l1 <= blockposition.getY() + 1 + i && flag; ++l1) {
                ...
                for (i1 = blockposition.getX() - k1; i1 <= blockposition.getX() + k1 && flag; ++i1) {
                    for (j1 = blockposition.getZ() - k1; j1 <= blockposition.getZ() + k1 && flag; ++j1) {
                        if (l1 >= 0 && l1 < 256) {
                            if (!this.canGrowInto(world.getBlockState(blockposition_mutableblockposition.setPos(i1, l1, j1)).getBlock())) {
                                flag = false;
                            }
                        } else {
                            flag = false;
            ...
            if (!flag) {
                ...
                if ((block == Blocks.GRASS || block == Blocks.DIRT) && blockposition.getY() < 256 - i - 1) {
                            ...
                            for (int j2 = blockposition.getZ() - k1; j2 <= blockposition.getZ() + k1; ++j2) {
                                ...
                                if (Math.abs(j1) != k1 || Math.abs(k2) != k1 || k1 <= 0) {
                                    ...
                                    if (!world.getBlockState(blockposition1).isFullBlock()) {
                                        this.setBlockAndNotifyAdequately(world, blockposition1, WorldGenTaiga1.LEAF);
                                    }
                                }
                            }
                        }
                    ...
                    for (i2 = 0; i2 < i - 1; ++i2) {
                        Material material = world.getBlockState(blockposition.up(i2)).getMaterial();

                        if (material == Material.AIR || material == Material.LEAVES) {
                            this.setBlockAndNotifyAdequately(world, blockposition.up(i2), WorldGenTaiga1.TRUNK);
                        }
                    }
