                            ...
                            for (int i4 = l2; i4 < i3; ++i4) {
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D) {
                                    for (int j4 = k2; j4 > j2; --j4) {
                                        ...
                                        if ((d12 * d12 + d13 * d13) * (double) this.rs[j4 - 1] + d14 * d14 / 6.0D < 1.0D) {
                                            ...
                                            if (iblockdata1.getBlock() == Blocks.STONE || iblockdata1.getBlock() == Blocks.DIRT || iblockdata1.getBlock() == Blocks.GRASS) {
                                                if (j4 - 1 < 10) {
                                                    chunksnapshot.setBlockState(j3, j4, i4, MapGenRavine.FLOWING_LAVA);
                                                } else {
                                                    chunksnapshot.setBlockState(j3, j4, i4, MapGenRavine.AIR);
                                                    if (flag2 && chunksnapshot.getBlockState(j3, j4 - 1, i4).getBlock() == Blocks.DIRT) {
                                                        blockposition_mutableblockposition.setPos(j3 + j * 16, 0, i4 + k * 16);
                                                        chunksnapshot.setBlockState(j3, j4 - 1, i4, this.world.getBiome(blockposition_mutableblockposition).topBlock);
                                                    }
                                                }
                                            }
                                        }
    ...
    protected void recursiveGenerate(World world, int i, int j, int k, int l, ChunkPrimer chunksnapshot) {
        if (this.rand.nextInt(50) == 0) {
            ...
            for (int i1 = 0; i1 < 1; ++i1) {
                ...
                this.addTunnel(this.rand.nextLong(), k, l, chunksnapshot, d0, d1, d2, f2, f, f1, 0, 0, 3.0D);
            }
