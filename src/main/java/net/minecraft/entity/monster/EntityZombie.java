...
public class EntityZombie extends EntityMob {
    ...
    private final AttributeModifier BABY_SPEED_BOOST = new AttributeModifier(EntityZombie.BABY_SPEED_BOOST_ID, "Baby speed boost", world.paperConfig.babyZombieMovementSpeed, 1); // Paper - Remove static - Make baby speed configurable
    ...
    protected void applyEntityAttributes() {
        ...
        this.getAttributeMap().registerAttribute(EntityZombie.SPAWN_REINFORCEMENTS_CHANCE).setBaseValue(this.rand.nextDouble() * 0.10000000149011612D);
    }

    protected void entityInit() {
        super.entityInit();
        this.getDataManager().register(EntityZombie.IS_CHILD, Boolean.valueOf(false));
        this.getDataManager().register(EntityZombie.VILLAGER_TYPE, Integer.valueOf(0));
        this.getDataManager().register(EntityZombie.ARMS_RAISED, Boolean.valueOf(false));
    }

    public void setArmsRaised(boolean flag) {
        this.getDataManager().set(EntityZombie.ARMS_RAISED, Boolean.valueOf(flag));
    }
    ...
    public boolean isChild() {
        return ((Boolean) this.getDataManager().get(EntityZombie.IS_CHILD)).booleanValue();
    }
    ...
    public void setChild(boolean flag) {
        this.getDataManager().set(EntityZombie.IS_CHILD, Boolean.valueOf(flag));
        if (this.world != null && !this.world.isRemote) {
            IAttributeInstance attributeinstance = this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED);

            attributeinstance.removeModifier(this.BABY_SPEED_BOOST);
            if (flag) {
                attributeinstance.applyModifier(this.BABY_SPEED_BOOST);
            }
        }
    ...
    public void notifyDataManagerChange(DataParameter<?> datawatcherobject) {
        if (EntityZombie.IS_CHILD.equals(datawatcherobject)) {
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float f) {
        if (super.attackEntityFrom(damagesource, f)) {
            ...
            if (entityliving == null && damagesource.getTrueSource() instanceof EntityLivingBase) {
                entityliving = (EntityLivingBase) damagesource.getTrueSource();
            }

            if (entityliving != null && this.world.getDifficulty() == EnumDifficulty.HARD && (double) this.rand.nextFloat() < this.getEntityAttribute(EntityZombie.SPAWN_REINFORCEMENTS_CHANCE).getAttributeValue() && this.world.getGameRules().getBoolean("doMobSpawning")) {
                ...
                for (int l = 0; l < 50; ++l) {
                    ...
                    if (this.world.getBlockState(new BlockPos(i1, j1 - 1, k1)).isTopSolid() && !this.world.isLightLevel(new BlockPos(i1, j1, k1), 10)) { // Paper
                        entityzombie.setPosition((double) i1, (double) j1, (double) k1);
                        if (!this.world.isAnyPlayerWithinRangeAt((double) i1, (double) j1, (double) k1, 7.0D) && this.world.checkNoEntityCollision(entityzombie.getEntityBoundingBox(), (Entity) entityzombie) && this.world.getCollisionBoxes(entityzombie, entityzombie.getEntityBoundingBox()).isEmpty() && !this.world.containsAnyLiquid(entityzombie.getEntityBoundingBox())) {
                            ...
                            this.getEntityAttribute(EntityZombie.SPAWN_REINFORCEMENTS_CHANCE).applyModifier(new AttributeModifier("Zombie reinforcement caller charge", -0.05000000074505806D, 0));
                            entityzombie.getEntityAttribute(EntityZombie.SPAWN_REINFORCEMENTS_CHANCE).applyModifier(new AttributeModifier("Zombie reinforcement callee charge", -0.05000000074505806D, 0));
                            break;
                        }
                    }
                }
    ...
    public static void registerFixesZombie(DataFixer dataconvertermanager) {
        EntityLiving.registerFixesMob(dataconvertermanager, EntityZombie.class);
    }
    ...
    public void onKillEntity(EntityLivingBase entityliving) {
        ...
        if ((this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD) && entityliving instanceof EntityVillager) {
            ...
            entityzombievillager.onInitialSpawn(this.world.getDifficultyForLocation(new BlockPos(entityzombievillager)), new EntityZombie.GroupData(false, null));
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficultydamagescaler, @Nullable IEntityLivingData groupdataentity) {
        ...
        if (object == null) {
            object = new EntityZombie.GroupData(this.world.rand.nextFloat() < 0.05F, null);
        }
        ...
        if (object instanceof EntityZombie.GroupData) {
            ...
            if (entityzombie_groupdatazombie.isChild) {
                ...
                if ((double) this.world.rand.nextFloat() < 0.05D) {
                    List list = this.world.getEntitiesWithinAABB(EntityChicken.class, this.getEntityBoundingBox().grow(5.0D, 3.0D, 5.0D), EntitySelectors.IS_STANDALONE);
        ...
        if (this.rand.nextFloat() < f * 0.05F) {
            this.getEntityAttribute(EntityZombie.SPAWN_REINFORCEMENTS_CHANCE).applyModifier(new AttributeModifier("Leader zombie bonus", this.rand.nextDouble() * 0.25D + 0.5D, 0));
