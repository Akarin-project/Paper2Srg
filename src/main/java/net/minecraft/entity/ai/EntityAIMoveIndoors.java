...
public class EntityAIMoveIndoors extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if ((!this.entity.world.isDaytime() || this.entity.world.isRaining() && !this.entity.world.getBiome(blockposition).canRain()) && this.entity.world.provider.hasSkyLight()) {
            if (this.entity.getRNG().nextInt(50) != 0) {
                ...
                Village village = this.entity.world.getVillageCollection().getNearestVillage(blockposition, 14);

                if (village == null) {
                    return false;
                } else {
                    this.doorInfo = village.getDoorInfo(blockposition);
    ...
    public void startExecuting() {
        this.insidePosX = -1;
        BlockPos blockposition = this.doorInfo.getInsideBlockPos();
        int i = blockposition.getX();
        int j = blockposition.getY();
        int k = blockposition.getZ();

        if (this.entity.getDistanceSq(blockposition) > 256.0D) {
